...
export const MainGamePage: React.FC<AllRoutesProps> = ({ socket }) => {
    ...
    socket.on("newAlphabet", async (alphabet: string) => {
        setLetter((e) => (e = alphabet));
    });

    // listen to updateTimer event
    socket.on("updatedTimer", async (timer: number) => {
        setTimer((e) => (e = timer));
    });

    // listen to score updates
    socket.on("updatedScore", async (game: Game) => {
        const opponentScore =
            game.player_1.socketId === socket.id
                ? game.player_2.score
                : game.player_1.score;
        setOpponentsScore((e: number) => (e = opponentScore));
    });
    ...
    return (
        <div className="relative">
            {alertBox && (
                <div
                    className="alertWarning absolute right-[20%] sm:right-[45%] transition duration-500 m-auto text-center p-4 mb-4 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300 text-lg"
                    role="alert"
                >
                    <span className="font-medium">All fields required!</span>
                </div>
            )}
            <div className=" text-white font-[cursive] flex items-center py-10 sm:py-40">
                <GameCard
